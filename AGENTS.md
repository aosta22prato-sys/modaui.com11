...
Hello successor Agent! You have been deployed to build, refine, or debug **MODAUI** (Enterprise Operating Platform). 
This project has already departed from traditional static web shops and has been transformed into a comprehensive **AI-Powered Enterprise Operating Platform**. To guarantee consecutive project integrity, prevent redundant component creations, and adhere to strict USER DESIGN INTENSE, you **MUST** read and memorize these architectural rules before changing any line of code.

---

## 🎯 Project Scope & User Design Philosophy
**MODAUI is an Enterprise Operating Platform (EOP). It lets user build and run a complete automated company.**
...
## 🏛️ Unified System Strategy (The Absorption Mandate)
**MODAUI is the SOLE intended operating system.** Stocky (located in `/script/`) is a legacy capability source that is being systematically absorbed.
1. **Single Truth Source (SSOT)**: 
   - Product/SKU Master = MODAUI.
   - Inventory Truth = MODAUI Kernel (`InventoryRecord`).
   - Financial Truth = MODAUI Kernel (`CompanyAccount`).
2. **Event-Driven Architecture**: All business operations (Sale, Purchase, POS) MUST emit events (`InventoryMovement`, `BusinessTransaction`) rather than directly modifying state.
3. **Legacy Decommissioning**: Any logic built in MODAUI that replicates a Stocky feature marks that Stocky feature for deletion. Do not build "bridges" for the sake of coexistence; build for replacement.
