...
export default function UnifiedArchitectureBridge({ isOpen, onClose, currentStep, adminMode, onNavigate }: UnifiedArchitectureBridgeProps) {
  ...
  const layers = [
    ...
    },
    {
      ...
      nodes: [
        ...
        { name: '快速登录入口', action: { step: 'CHOOSE_INDUSTRY', trigger: 'login' } },
    ...
    },
    {
      ...
      title: '3. 商家主后台 (Merchant Admin)',
      subtitle: '全渠道AI联合协作面板',
      color: 'border-emerald-500/30 text-emerald-400 bg-emerald-950/25',
      nodes: [
        { name: '运营大盘与智能指挥舱', action: { step: 'DASHBOARD', activeMenu: 'workbench' } },
        { name: 'AI团队协作指挥舱', action: { step: 'DASHBOARD', activeMenu: 'workbench' } },
        { name: '商品SPU录入中心', action: { step: 'DASHBOARD', activeMenu: 'product' } },
        { name: '多渠道订单与物流', action: { step: 'DASHBOARD', activeMenu: 'order' } },
        { name: '客户成功与赔付', action: { step: 'DASHBOARD', activeMenu: 'customer' } },
        { name: '活动营销策略', action: { step: 'DASHBOARD', activeMenu: 'marketing' } },
        { name: '经营数据与财务看板', action: { step: 'DASHBOARD', activeMenu: 'analytics' } },
        { name: '模型与算力策略设置', action: { step: 'DASHBOARD', activeMenu: 'settings' } }
      ]
    },
    {
      id: 'CUSTOMER_STOREFRONT',
      title: '4. 客户商城前台 (Customer Mall)',
      subtitle: '支持多元视效装修的C端商城',
      color: 'border-rose-500/30 text-rose-400 bg-rose-950/25',
      nodes: [
        { name: '商城首页', action: { step: 'CUSTOMER_STOREFRONT', tab: 'home' } },
        { name: '分类选品', action: { step: 'CUSTOMER_STOREFRONT', tab: 'menu' } },
        { name: '购物车与优惠券', action: { step: 'CUSTOMER_STOREFRONT', tab: 'cart' } },
        { name: '结账付款', action: { step: 'CUSTOMER_STOREFRONT', tab: 'checkout' } },
        { name: 'AI客服中心', action: { step: 'CUSTOMER_STOREFRONT', tab: 'menu', chat: true } },
        { name: '会员订单核销', action: { step: 'CUSTOMER_STOREFRONT', tab: 'success' } }
      ]
    },
    {
      id: 'AI_TEAMS',
    ...
    {
      id: 'PLATFORM_ADMIN',
      title: '8. Super SaaS 平台总控 (Platform Controller)',
      subtitle: '管理多租户多商铺与算力流水配额',
      color: 'border-violet-500/30 text-violet-400 bg-violet-950/25',
      nodes: [
        { name: 'SaaS监管大盘', action: { step: 'PLATFORM_ADMIN', tab: 'dashboard' } },
        { name: '租户与商户管理', action: { step: 'PLATFORM_ADMIN', tab: 'shops' } },
        { name: '行业与AI团队配置', action: { step: 'PLATFORM_ADMIN', tab: 'teams' } },
        { name: '任务与服务调度监控', action: { step: 'PLATFORM_ADMIN', tab: 'tasks' } },
        { name: 'API Key与配额策略', action: { step: 'PLATFORM_ADMIN', tab: 'settings' } }
      ]
    },
    {
      id: 'SYSTEM_BASE',
      ...
      nodes: [
        { name: '模拟账户与RBAC权限', action: { step: 'PLATFORM_ADMIN', view: 'rbac' } },
        { name: '支付通道与对账日志', action: { step: 'PLATFORM_ADMIN', view: 'payment' } },
        { name: 'Websocket与事件队列', action: { step: 'PLATFORM_ADMIN', view: 'socket' } },
        { name: '实时流量与防刷防护', action: { step: 'PLATFORM_ADMIN', view: 'firewall' } },
        { name: '对象存储浏览器', action: { step: 'PLATFORM_ADMIN', view: 'storage' } }
      ]
    }
  ];
  ...
  filteredLayers = layers.map((layer) => ({
    ...
    nodes: layer.nodes.filter((node) => {
      ...
      return node.name.toLowerCase().includes(keyword) || layer.title.toLowerCase().includes(keyword) || layer.subtitle.toLowerCase().includes(keyword);
    }),
  })).filter((layer) => layer.nodes.length > 0 || !searchQuery.trim());
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 font-sans scrollbar-thin scrollbar-thumb-neutral-800 bg-black">
          ...
          {activeTab === 'shuttle' ? (
            <div className="space-y-6">
              <div className="p-4 rounded-xl border border-[#2F3336] bg-neutral-950/60 leading-relaxed text-xs text-neutral-400 flex flex-col gap-4">
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                  <input
                    ...
                    placeholder="搜索传送目的地，例如：后台、AI、知识库、前端店面"
                    className="w-full rounded-2xl border border-[#2F3336] bg-[#09090D] text-sm text-white px-4 py-3 outline-none focus:border-[#1D9BF0]"
                  />
                  <button
                    onClick={() => setSearchQuery('')}
                    className="rounded-2xl border border-neutral-800 bg-neutral-900 px-4 py-3 text-sm text-neutral-300 hover:text-white hover:border-[#1D9BF0] transition-colors"
                  >清空</button>
                </div>
                <div className="text-[10px] text-neutral-500 font-mono">快速传送舱已启动：输入关键词即可在全景架构树内定位目标，并一键去往任何已实现页面。</div>
              </div>
              ...
              <div className="relative space-y-6 lg:space-y-8 pl-4 border-l border-[#2F3336]/60">
                {filteredLayers.length > 0 ? (
                  ...
                  <div className="rounded-2xl border border-rose-500/20 bg-rose-950/40 p-6 text-sm text-rose-300">
                    未找到匹配的传送目的地：<span className="font-semibold text-white">{searchQuery}</span>。
                    <div className="mt-2 text-xs text-neutral-400">请尝试更短关键词，例如“后台”、“AI”、“知识库”、“店面”等。</div>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="border border-[#2F3336] rounded-xl overflow-hidden bg-neutral-950/80">
                  <table className="w-full text-left border-collapse text-[11px]">
                    ...
                    <tbody className="divide-y divide-[#2F3336]/60 font-mono text-zinc-400">
                      ...
                      <tr>
                        <td className="p-3 font-bold text-white">10. 银行与理财金服</td>
                        <td className="p-3">全量集成 Laravel 收银微服务（含 P2P 极速转账、虚拟卡签发、工单卡券与黑金插画库）。</td>
                        <td className="p-3 text-right text-emerald-400">100% 🟢</td>
                      </tr>
                    </tbody>
                  </table>
