...
export default function StorefrontView({ tenantId, industryId, onAddLog }: StorefrontViewProps) {
  ...
  useEffect(() => {
    ...
    const fetchStoreSettings = async () => {
      try {
        const res = await apiService.getStore(activeTenant);
        if (res.success && res.store) {
          const branding = res.store.branding || {};
          if (branding.storeTheme) setSelectedThemeId(branding.storeTheme);
          if (branding.storeHeadline) setStoreHeadline(branding.storeHeadline);
          if (branding.phoneContact) setPhoneContact(branding.phoneContact);
          if (branding.deliveryType) setDeliveryType(branding.deliveryType);
          if (branding.logoImage) setLogoImage(branding.logoImage);
        }
      } catch (err) {
        console.warn("StorefrontView settings fetch failed:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Apply Changes with full save to Backend
  const handleApplyThemeSettings = async () => {
    setIsSaving(true);
    try {
      const activeTenant = tenantId || 'default_tenant';
      const res = await apiService.updateStore(activeTenant, {
        branding: {
      ...
      if (res.success && onAddLog) {
        onAddLog('AI视觉主编', '🏪', `一键应用了网店最新视觉系统【${selectedThemeId.toUpperCase()}】，全量推流并刷新了 SQLite 节点。`, 'success');
      }
    } catch (err: any) {
        ...
        <div className="bg-[#09090B] border border-[#2F3336] p-5 rounded-xl space-y-4">
            ...
            <div className="sm:col-span-2 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2.5">
                {(['takeout', 'delivery', 'dine_in'] as const).map((mode) => (
                  <Button
                    key={mode}
                    variant={deliveryType === mode ? 'primary' : 'outline'}
                    size="sm"
                    onClick={() => setDeliveryType(mode)}
                    className={`font-mono text-[10px] ${deliveryType === mode ? '' : 'text-zinc-400 border-zinc-900 hover:text-white'}`}
                  >
          ...
          <div className="pt-3 border-t border-zinc-900/60 flex justify-end">
            <Button
              ...
              disabled={isSaving}
              variant="secondary"
              className="bg-white hover:bg-neutral-200 transition-colors text-black font-extrabold text-[10px] py-1.5 px-6 flex items-center space-x-1 disabled:opacity-45"
            >
              {isSaving ? (
                <>
