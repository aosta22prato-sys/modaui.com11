  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const storeRes = await apiService.getStore(localTenantId);
        if (storeRes.success && storeRes.store) {
          const branding = storeRes.store.branding || {};
          if (branding.merchantName) setCompany(branding.merchantName);
          if (branding.storeHeadline) setHeadline(branding.storeHeadline);
          if (branding.storeTheme) setTheme(branding.storeTheme);
        }

        // Fetch Products
        const productRes = await apiService.getProducts(localTenantId);
        if (productRes.success && productRes.products && productRes.products.length > 0) {
          setProducts(productRes.products.map((p: any) => ({
            ...p,
            stock: p.inventory, // Map inventory to stock for UI
            image: p.image || '📦'
          })));
        } else {
          // Fallback
        ...
        if (cartRes.success && cartRes.cart) {
          setCustomerCart(cartRes.cart.items || []);
          setCouponApplied(!!cartRes.cart.coupon);
        }
      } catch (error) {
        console.warn("Failed to fetch storefront data:", error);
      }
    };
    ...
    const syncCart = async () => {
      if (customerCart.length >= 0) {
        await apiService.syncCart({
          items: customerCart,
          coupon: couponApplied ? 'WELCOME12' : undefined,
          discount: couponApplied ? 12 : 0
        });
      }
    };
    const timer = setTimeout(syncCart, 1000);
    return () => clearTimeout(timer);
  }, [customerCart, couponApplied]);
    ...
    <div className="min-h-screen bg-[#09090C] text-zinc-100 flex flex-col font-sans overflow-x-hidden select-none">
      ...
      <header className="bg-zinc-950 border-b border-zinc-800 py-3.5 px-6 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 relative z-50 shadow-md">
        ...
        <div className="flex items-center space-x-3 shrink-0">
          <Button onClick={handleBackToDashboard} variant="secondary" size="sm" className="text-xs">
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {filteredProducts.map((p) => (
                      <Card
                        key={p.id}
                        className={`p-3 rounded-3xl flex flex-col justify-between hover:scale-[1.01] duration-150 cursor-pointer ${currentStyle.accentBg} ${currentStyle.accentBorder}`}
                      >
                        <div className="relative">
                          <Badge variant="warning" className="absolute top-0 left-0 shadow-lg">
                            REC
                          </Badge>
                        ...
                        try {
                          if (selectedPayMethod === 'stripe') {
                            ...
                            if (orderRes.success) {
                              ...
                              await apiService.paymentCallback(orderRes.payment.id, {
                                status: 'success',
                            ...
                            if (orderRes.success) {
                              // In real world, we would call a real wechat pay api here
                              // For now we simulate getting a QR
                              setWechatQrUrl('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=WECHAT_PAY_SIMULATION');
                              setCurrentPaymentOrderId(orderRes.order.id);
                            ...
                            if (orderRes.success) {
                              setPaypalApprovalLink('https://www.paypal.com/checkoutnow?token=SIMULATED_TOKEN');
                              setCurrentPaymentOrderId(orderRes.order.id);
                              setCurrentPaymentStatus('pending');
                              window.open('https://www.paypal.com/checkoutnow?token=SIMULATED_TOKEN', '_blank');
