...
export default function LandingPage({ onStartFlow, onSelectIndustry, onNavigatePlatformAdmin }: LandingPageProps) {
  ...
  const navigateToAdmin = () => {
    if (onNavigatePlatformAdmin) {
      onNavigatePlatformAdmin();
    } else if (adminEntryUrl) {
    ...
    <div className="bg-black text-[#FFFFFF] font-sans selection:bg-[#1D9BF0] selection:text-white">
      ...
      <nav className="border-b border-[#2F3336] bg-black/90 backdrop-blur sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 sm:space-x-6 text-sm font-medium text-[#8B949E]">
            {onNavigatePlatformAdmin && (
              <Button 
                onClick={navigateToAdmin}
                className="hover:text-amber-400 text-amber-500 font-bold font-mono transition-colors cursor-pointer text-xs flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-md"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
                {t('platform_admin_center')}
              </Button>
            )}
            <Button 
              onClick={() => onStartFlow()}
      ...
      <section className="relative overflow-hidden pt-20 pb-24 md:pt-28 md:pb-32 border-b border-[#2F3336]">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10 space-y-10">
          ...
          {/* DUAL HERO ENTRANCES: MERCHANT SIGNUP VS PLATFORM PROPRIETOR ADMIN */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Button
              onClick={() => onStartFlow()}
              className="w-full sm:w-auto bg-[#1D9BF0] text-white px-8 py-4 rounded-full text-md font-bold hover:bg-[#38BDF8] transition-all transform active:scale-95 shadow-lg shadow-[#1D9BF0]/10 duration-205 flex items-center justify-center space-x-2.5 border border-transparent cursor-pointer"
            >
              <span>{t('create_my_company')}</span>
            ...
            {onNavigatePlatformAdmin && (
              <Button
                onClick={navigateToAdmin}
                className="w-full sm:w-auto bg-zinc-950 hover:bg-zinc-900 text-amber-400 border border-amber-500/35 hover:border-amber-400 px-8 py-4 rounded-full text-md font-bold transition-all transform active:scale-95 shadow-lg hover:shadow-amber-500/5 duration-205 flex items-center justify-center space-x-2.5 cursor-pointer font-mono"
              >
                <span className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse" />
                <span>{t('modaui_platform_center')}</span>
              </Button>
            )}
          </motion.div>
          ...
          {INDUSTRIES.map((ind, idx) => (
            ...
            >
              ...
              <div className="mt-6 pt-4 border-t border-[#2F3336]/60 flex items-center justify-between text-[10px] font-mono text-[#8B949E]">
                <span>{t('team_6_members')}</span>
                <span className="text-[#1D9BF0]">{t('one_click_start')} →</span>
              </div>
            </motion.div>
          ))}
        </div>
