...
export default function SystemBaseView({ onBackToLanding }: { onBackToLanding: () => void }) {
  ...
  const [swaggerResponse, setSwaggerResponse] = useState<string>(t('systemBase.swaggerIdle'));
  ...
  const handleTestAPIEndpoint = (endpoint: string) => {
    ...
    setSwaggerResponse(t('systemBase.swaggerCalling', { endpoint }));

    setTimeout(() => {
      setIsCallingAPI(false);
      setSwaggerResponse(t('systemBase.swaggerSuccess', { requestId: `req_sh_92a9b${Math.floor(Math.random() * 90000) + 10000}cfff` }));
    }, 1200);
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white py-12 px-6 relative font-sans">
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-[#2F3336] pb-6 gap-4">
          <div>
            ...
            >
              ...
              <span>{t('systemBase.backToConsole')}</span>
            </button>
          ...
          <div className="flex items-center space-x-4 bg-[#09090C] p-3 rounded-lg border border-[#2F3336] shrink-0 font-mono text-[10px] text-neutral-400">
            <div className="flex items-center space-x-1.5">
              ...
              <span>{t('systemBase.apiSecurity', { tier: 'WAF_TIER_3' })}</span>
            </div>
            <div className="h-4 w-[1px] bg-neutral-800" />
            <div className="flex items-center space-x-1.5">
              <Terminal className="w-4 h-4 text-[#1D9BF0]" />
              <span>{t('systemBase.defaultPort', { port: 3000 })}</span>
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-neutral-800 flex flex-wrap gap-1">
          {[
            { id: 'rbac', name: t('systemBase.tab.rbac') },
            { id: 'api', name: t('systemBase.tab.api') },
            { id: 'firewall', name: t('systemBase.tab.firewall') },
            { id: 'queue', name: t('systemBase.tab.queue') },
            { id: 'bucket', name: t('systemBase.tab.bucket') }
          ].map((tb) => (
            <button
              key={tb.id}
            ...
            {activeTab === 'rbac' && (
              ...
              >
                <div className="flex justify-between items-center pb-2.5 border-b border-[#2F3336]/60">
                  <h3 className="text-sm font-bold text-white">{t('systemBase.rbacHeading')}</h3>
                  ...
                  <table className="w-full text-left text-xs font-sans divide-y divide-neutral-800">
                    <thead>
                      <tr className="text-neutral-500 font-mono text-[10px]">
                        <th className="pb-3 font-bold">{t('systemBase.table.role')}</th>
                        <th className="pb-3 font-bold">{t('systemBase.table.scope')}</th>
                        <th className="pb-3 font-bold">{t('systemBase.table.createShop')}</th>
                        <th className="pb-3 font-bold">{t('systemBase.table.deployAI')}</th>
                        <th className="pb-3 font-bold">{t('systemBase.table.auditFinancials')}</th>
                        <th className="pb-3 font-bold">{t('systemBase.table.controls')}</th>
                      </tr>
                    </thead>
