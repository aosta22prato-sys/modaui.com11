...
export default function RoleManagementPanel({ 
  ...
  const loadMembers = async () => {
    try {
      const res = await apiService.getMembers(tenantId);
      if (res.success) {
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiService.addMember(tenantId, {
        name: newMemberName,
      ...
      if (res.success) {
        ...
        if (onAddLog) {
          onAddLog('RBAC 权限守卫', '🛡️', `成功添加用户 ${res.member.name} 并预设「${res.member.role}」RBAC 权限策略。`, 'success');
        }
      }
    } catch (error) {
  ...
  const handleToggleStatus = async (memberId: string) => {
    ...
    try {
      const res = await apiService.updateMemberStatus(tenantId, memberId, nextStatus);
      if (res.success) {
        setMembers(members.map(m => m.id === memberId ? res.member : m));
        if (onAddLog) {
  ...
  const handleDeleteMember = async (memberId: string, memberName: string) => {
    try {
      const res = await apiService.deleteMember(tenantId, memberId);
      if (res.success) {
